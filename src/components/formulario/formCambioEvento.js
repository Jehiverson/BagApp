...
export const FormCambioEvento = () => {
  ...
  const [selectedClientePago, setSelectedClientePago] = useState('');
  const [actividades, setActividades] = useState('');
  const [selectedClienteCambio, setSelectedClienteCambio] = useState('');
  ...
  useEffect(() => {
    async function DatosPagos() {
      try {
        ...
        const pagoFormato = pagoData
          .filter(pago => pago.idCliente === idCliente) // Filtrar por idCliente
          .map(pago => ({
            value: pago.idPago,
            label: pago.actividad.nombreActividad
          }));
    ...
    async function getActividades() {
        try {
            ...
            setActividades(actividadesFormatted); // Update the actividades state
        } catch (error) {
